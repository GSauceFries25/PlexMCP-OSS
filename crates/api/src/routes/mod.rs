...
use crate::{
    auth::{optional_auth, require_auth},
    state::AppState,
...
pub fn create_router(state: AppState) -> Router {
    ...
    let mut public_api_routes = Router::new()
        ...
        .route(
            "/audit/oauth-session-created",
            post(audit::oauth_session_created),
        )
        ...
        .route(
            "/public/enterprise-inquiry",
            post(public::submit_enterprise_inquiry),
        )
        // Website analytics collection (public with optional auth for admin exclusion)
        .route(
            "/analytics/collect",
            post(analytics_tracking::collect).route_layer(middleware::from_fn_with_state(
                auth_state.clone(),
                optional_auth,
            )),
        );
    ...
    let mut protected_api_routes = Router::new()
        ...
        .route(
            "/org/subdomain/check",
            post(organizations::check_subdomain_availability),
        )
        // User routes
        ...
        .route(
            "/mcps/:mcp_id/health-check",
            post(mcps::trigger_health_check),
        )
        ...
        .route(
            ...
            post(two_factor::regenerate_backup_codes),
        )
        // Trusted device management
        .route("/2fa/devices", get(two_factor::list_trusted_devices))
        .route(
            "/2fa/devices",
            delete(two_factor::revoke_all_trusted_devices),
        )
        .route(
            "/2fa/devices/:device_id",
            delete(two_factor::revoke_trusted_device),
        )
        ...
        .route(
            "/account/identities/providers",
            get(identities::list_providers),
        )
        .route(
            "/account/identities/:provider",
            delete(identities::unlink_identity),
        )
        ...
        .route(
            "/notification-preferences",
            patch(notifications::update_notification_preferences),
        )
        ...
        .route(
            ...
            post(support::reply_to_ticket),
        )
        .route(
            "/support/tickets/:ticket_id/close",
            post(support::close_ticket),
        )
        ...
        .route(
            "/admin/users/:user_id/disable-2fa",
            post(admin::disable_user_2fa),
        )
        ...
        .route(
            ...
            delete(admin::revoke_user_api_key),
        )
        // Admin overages toggle routes
        .route("/admin/orgs/:org_id/overages", get(admin::get_org_overages))
        .route(
            "/admin/orgs/:org_id/overages",
            put(admin::toggle_org_overages),
        )
        ...
        .route(
            "/admin/support/stats/enhanced",
            get(support::admin_get_ticket_stats_enhanced),
        )
        ...
        .route(
            ...
            get(support::admin_get_ticket).patch(support::admin_update_ticket),
        )
        .route(
            "/admin/support/tickets/:ticket_id/reply",
            post(support::admin_reply_to_ticket),
        )
        .route(
            "/admin/support/tickets/:ticket_id/reply-internal",
            post(support::admin_reply_with_internal),
        )
        .route(
            "/admin/support/tickets/:ticket_id/assign",
            post(support::admin_assign_ticket),
        )
        .route(
            "/admin/support/tickets/:ticket_id/history",
            get(support::admin_get_assignment_history),
        )
        .route(
            "/admin/support/tickets/batch/assign",
            post(support::admin_batch_assign),
        )
        .route(
            "/admin/support/tickets/batch/status",
            post(support::admin_batch_status),
        )
        // SLA rules management
        .route(
            "/admin/support/sla/rules",
            get(support::admin_list_sla_rules),
        )
        .route(
            "/admin/support/sla/rules",
            post(support::admin_create_sla_rule),
        )
        .route(
            "/admin/support/sla/rules/:rule_id",
            patch(support::admin_update_sla_rule),
        )
        // Template management
        .route(
            "/admin/support/templates",
            get(support::admin_list_templates),
        )
        .route(
            "/admin/support/templates",
            post(support::admin_create_template),
        )
        .route(
            "/admin/support/templates/:template_id",
            patch(support::admin_update_template),
        )
        .route(
            "/admin/support/templates/:template_id",
            delete(support::admin_delete_template),
        )
        // Admin website analytics routes
        .route(
            "/admin/analytics/website/realtime",
            get(analytics_tracking::get_realtime),
        )
        .route(
            "/admin/analytics/website/overview",
            get(analytics_tracking::get_overview),
        )
        .route(
            "/admin/analytics/website/overview-enhanced",
            get(analytics_tracking::get_overview_enhanced),
        )
        .route(
            "/admin/analytics/website/timeseries",
            get(analytics_tracking::get_timeseries),
        )
        .route(
            "/admin/analytics/website/pages",
            get(analytics_tracking::get_top_pages),
        )
        .route(
            "/admin/analytics/website/referrers",
            get(analytics_tracking::get_referrers),
        )
        .route(
            "/admin/analytics/website/devices",
            get(analytics_tracking::get_devices),
        )
        .route(
            "/admin/analytics/website/locations",
            get(analytics_tracking::get_locations),
        )
        .route(
            "/admin/analytics/website/events",
            get(analytics_tracking::get_events),
        )
        .route(
            "/admin/analytics/website/events/details",
            get(analytics_tracking::get_event_details),
        )
        .route(
            "/admin/analytics/website/goals",
            get(analytics_tracking::list_goals),
        )
        .route(
            "/admin/analytics/website/goals",
            post(analytics_tracking::create_goal),
        )
        .route(
            "/admin/analytics/website/goals/:goal_id",
            patch(analytics_tracking::update_goal),
        )
        .route(
            "/admin/analytics/website/goals/:goal_id",
            delete(analytics_tracking::delete_goal),
        )
        .route(
            "/admin/analytics/website/settings",
            get(analytics_tracking::get_settings),
        )
        .route(
            "/admin/analytics/website/settings",
            patch(analytics_tracking::update_settings),
        )
        .route(
            "/admin/analytics/website/alerts",
            get(analytics_tracking::list_alerts),
        )
        .route(
            "/admin/analytics/website/alerts/:alert_id/resolve",
            patch(analytics_tracking::resolve_alert),
        )
    ...
    if state.config.enable_billing {
        protected_api_routes = protected_api_routes
            ...
            .route(
                "/billing/subscription/preview-proration",
                get(billing::preview_proration),
            )
            // Overage routes
            .route("/billing/overages", get(billing::get_overages))
            .route(
                "/billing/overages/current",
                get(billing::get_current_overage),
            )
            .route(
                "/billing/overages/accumulated",
                get(billing::get_accumulated_overage),
            )
            ...
            .route(
                "/billing/instant-charges",
                get(billing::get_instant_charges),
            )
            ...
            .route(
                "/billing/subscription/downgrade",
                get(billing::get_scheduled_downgrade),
            )
            .route(
                "/billing/subscription/downgrade",
                delete(billing::cancel_scheduled_downgrade),
            )
            ...
            .route(
                ...
                get(billing::get_invoice_detail),
            )
            .route(
                "/billing/invoices/:invoice_id/pay",
                post(billing::pay_invoice),
            )
            .route(
                "/billing/invoices/:invoice_id/dispute",
                post(billing::create_invoice_dispute),
            )
            .route(
                "/billing/grace-period",
                get(billing::get_grace_period_status),
            )
            // Add-on routes
            ...
            .route(
                "/addons/:addon_type/quantity",
                patch(addons::update_addon_quantity),
            )
            ...
            .route(
                "/usage/latency-distribution",
                get(usage::get_latency_distribution),
            )
            // Admin billing diagnostic routes (requires billing feature)
            .route(
                "/admin/billing/invariants",
                get(admin::check_billing_invariants),
            )
            .route(
                "/admin/billing/debug/:org_id",
                get(admin::debug_org_billing),
            );
    }

    // Apply auth middleware to protected routes
    let protected_api_routes = protected_api_routes.layer(middleware::from_fn_with_state(
        auth_state.clone(),
        require_auth,
    ));

    // WebSocket routes (auth handled in handler via query parameter)
    let websocket_routes = Router::new().route("/ws/support", get(ws_handler));
    ...
    let mcp_routes = Router::new()
        ...
        .layer(DefaultBodyLimit::max(
            state.config.mcp_max_request_body_bytes,
        ));
