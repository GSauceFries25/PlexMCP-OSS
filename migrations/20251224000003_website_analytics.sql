-- PlexMCP In-House Website Analytics
-- Migration: 20251224000003_website_analytics.sql
-- Full Google Analytics replacement - zero third-party dependencies
...
CREATE INDEX IF NOT EXISTS idx_analytics_visitors_fingerprint ON analytics_visitors(fingerprint_hash);
CREATE INDEX IF NOT EXISTS idx_analytics_visitors_user ON analytics_visitors(user_id) WHERE user_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_analytics_visitors_first_seen ON analytics_visitors(first_seen_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_analytics_sessions_country ON analytics_sessions(country_code) WHERE country_code IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_analytics_sessions_device ON analytics_sessions(device_type);
CREATE INDEX IF NOT EXISTS idx_analytics_sessions_referrer ON analytics_sessions(referrer_domain) WHERE referrer_domain IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_analytics_sessions_utm ON analytics_sessions(utm_source, utm_medium) WHERE utm_source IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS idx_analytics_pageviews_visitor ON analytics_page_views(visitor_id);
CREATE INDEX IF NOT EXISTS idx_analytics_pageviews_path ON analytics_page_views(url_path);
CREATE INDEX IF NOT EXISTS idx_analytics_pageviews_entered ON analytics_page_views(entered_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_analytics_events_visitor ON analytics_events(visitor_id) WHERE visitor_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_analytics_events_name ON analytics_events(event_name);
CREATE INDEX IF NOT EXISTS idx_analytics_events_category ON analytics_events(event_category) WHERE event_category IS NOT NULL;
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_analytics_goals_name ON analytics_goals(name);
...
CREATE INDEX IF NOT EXISTS idx_analytics_conversions_session ON analytics_conversions(session_id) WHERE session_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_analytics_conversions_created ON analytics_conversions(created_at DESC);
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_analytics_agg_hour_path
ON analytics_aggregates_hourly(period_hour, COALESCE(page_path, ''));

CREATE INDEX IF NOT EXISTS idx_analytics_agg_hour ON analytics_aggregates_hourly(period_hour DESC);

-- =============================================================================
-- Analytics Settings (Privacy Controls)
-- =============================================================================
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_analytics_settings_singleton
ON analytics_settings((TRUE));

-- Insert default settings
INSERT INTO analytics_settings (anonymize_ip, collect_city, respect_dnt, raw_data_retention_days)
VALUES (TRUE, FALSE, TRUE, 90)
...
CREATE INDEX IF NOT EXISTS idx_analytics_realtime_activity ON analytics_realtime(last_activity_at DESC);
